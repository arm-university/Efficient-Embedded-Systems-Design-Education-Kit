...
unsigned int gpio_get_range(Pin pin_base, int count) {
	...
	return READ_BIT(p->IDR,(((1 << count) - 1)<<pin_index));
}
...
void gpio_set_callback(Pin pin, void (*callback)(int status)) {
  ...
  IRQ_status = 0;
	...
	switch(IRQ_pin_index){
		case 0:
			SYSCFG->EXTICR[0]&=IRQ_port_num<<IRQ_pin_index * 4;
    ...
    case 2:
		...
		break;		
		case 4:
			SYSCFG->EXTICR[1]&=IRQ_port_num<<IRQ_pin_index * 4;
    ...
    case 10:
		case 11:
			SYSCFG->EXTICR[2]&=IRQ_port_num<<IRQ_pin_index * 4;
		break;
    ...
    case 14:
		case 15:
			SYSCFG->EXTICR[3]&=IRQ_port_num<<IRQ_pin_index * 4;
		break;
	}
}
